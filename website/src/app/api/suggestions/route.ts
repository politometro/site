...
function requiresDiscordApproval(item: JsonRecord): boolean {
  const approval = isJsonRecord(item.discordApproval)
    ? item.discordApproval
    : null;
  const origin = sanitizeText(item.origin, 80).toLocaleLowerCase();
  const id = sanitizeText(item.id, 140).toLocaleLowerCase();
  return Boolean(
    approval?.required === true ||
      ["community", "discord", "discord-command", "website"].includes(origin) ||
      id.startsWith("web_") ||
      id.startsWith("discord_"),
  );
}

function hasDiscordApprovalProof(item: JsonRecord): boolean {
  if (!requiresDiscordApproval(item)) return true;
  const approval = isJsonRecord(item.discordApproval)
    ? item.discordApproval
    : null;
  if (!approval) return false;
  const messageId = sanitizeText(approval.messageId, 100);
  return Boolean(
    approval.required === true &&
      approval.status === "approved" &&
      messageId &&
      sanitizeText(approval.channelId, 100) &&
      messageId === sanitizeText(item.discordMessageId, 100) &&
      item.notificationStatus === "sent" &&
      sanitizeText(approval.sentAt, 100) &&
      sanitizeText(approval.approvedAt, 100) &&
      sanitizeText(approval.approvedBy, 100),
  );
}
...
function validateReplaceRecord(value: unknown, position: string): JsonRecord {
  ...
  if (
    position.startsWith("queue[") &&
    normalized.status === "queue" &&
    requiresDiscordApproval(normalized) &&
    !hasDiscordApprovalProof(normalized)
  ) {
    throw new ApiError(
      `${position}: uma sugestão comunitária não pode entrar na fila sem aprovação Discord verificável.`,
      400,
    );
  }
...
function isDuplicate(
  ...
  const incomingAuthor = sanitizeText(recommendation.authorOrMeta, 260);
  return (
    titleSimilarity(existingTitle, recommendation.title) >= 0.97 &&
    (!existingAuthor ||
      !incomingAuthor ||
      titleSimilarity(existingAuthor, incomingAuthor) >= 0.8)
  );
}
...
function unresolvedSubmission(
  ...
  return {
    ...
    verification: {
      ...
      matchedFields: link ? ["title", "submitted-link"] : ["title"],
      warnings: [],
    },
  };
}
...
async function resolveSubmission(
  ...
  try {
    const resolved = await resolveRecommendation({ type, title, link });
    if (validateResolvedRecommendation(resolved)) {
      // Project ideas do not need enrichment, but their optional public link
      // must still be retained for the Discord reviewer.
      return type === "project" && link ? { ...resolved, link } : resolved;
    }
  } catch {
    // Source enrichment is best-effort at intake. The raw, sanitised
    // suggestion still has to reach Discord and can be enriched after review.
  }
  return unresolvedSubmission(type, title, link);
}
...
async function notifyDiscord(item: JsonRecord): Promise<DiscordDelivery | null> {
  ...
  const fields: Array<{ name: string; value: string; inline: boolean }> = [
    ...
    },
    {
      name: "Autor / Fonte",
      value: isVerified
        ? truncateDiscord(item.authorOrMeta, 1_024)
        : "A validar após aprovação",
      inline: true,
    },
    {
      name: "Verificação",
      value: isVerified
        ? `${truncateDiscord(verification.provider, 80)} · ${confidence}`
        : "Pendente — a sugestão ainda não entra no workflow de posts",
      inline: true,
    },
  ];
  if (link) {
  ...
  const embed: JsonRecord = {
    ...
    description: isVerified
      ? truncateDiscord(item.description, 4_096)
      : "Sugestão submetida pela comunidade para análise editorial.",
    color: typeColors[type],
    fields,
    footer: {
      text: truncateDiscord(
        `ID: ${item.id || "sem-id"} | Aprovação obrigatória`,
        2_048,
      ),
    },
    timestamp:
  ...
  for (let attempt = 1; attempt <= 3; attempt += 1) {
    try {
      ...
      if (response.ok) {
        ...
        return typeof delivered.id === "string"
          ? {
...
async function appendSuggestion(payload: AppendPayload) {
  ...
  if (title.length < 3) {
    throw new ApiError("O título deve ter pelo menos 3 caracteres.", 400);
  }
  const rawLink =
    typeof payload.item.link === "string" ? payload.item.link.trim() : "";
  const normalizedLink = rawLink ? normalizePublicUrl(rawLink) : "";
  if (rawLink && !normalizedLink) {
    throw new ApiError("O link deve ser um URL HTTP(S) público e válido.", 400);
  }
  ...
  let readyToStore: ResolvedForStorage = resolved;
  if (validateResolvedRecommendation(resolved)) {
    try {
      readyToStore = await persistRecommendationCover(resolved);
    } catch {
      readyToStore = unresolvedSubmission(
        payload.item.type,
        title,
        normalizedLink || "",
      );
  ...
  const discordDelivery = await notifyDiscord(storedItem);
  const notificationSent = Boolean(discordDelivery);
    ...
    try {
      const marked = await mutateDatabase((database) => {
        ...
        const updated = {
          ...
          discordMessageId: discordDelivery!.messageId,
          discordNotifiedAt: discordDelivery!.sentAt,
          discordApproval: {
            required: true,
            status: "pending",
            channelId: discordDelivery!.channelId,
            messageId: discordDelivery!.messageId,
            sentAt: discordDelivery!.sentAt,
          },
        };
